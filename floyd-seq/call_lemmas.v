...
Fixpoint argtypes (al: list (ident * type)) : list type :=
  match al with nil => nil | (_, t) :: al' => t :: argtypes al' end.

Lemma argtypes_eq:
  forall al, argtypes al = snd (split al).
Proof.
...
Definition removeopt_localdef (ret: option ident)
  (l: list localdef) : list localdef :=
  match ret with
...
Lemma semax_call':
  forall Espec {cs: compspecs} Delta fs A Pre Post NEPre NEPost
         ts x ret argsig retsig cc a b id bl P Q R,
    ...
    forall (Hret: tc_fn_return Delta ret retsig)
      ...
      @semax cs Espec Delta
        ((tc_expr Delta a && tc_exprlist Delta (argtypes argsig) bl) &&
         ...
         (|> `(Pre ts x: environ -> mpred)
              (make_args' (argsig, retsig) (eval_exprlist (argtypes argsig) bl)) *
...
Proof.
  ...
  3:{
    ...
    Hsub Delta ts x (PROPx P (LOCALx Q (SEPx R))) ret a bl b id H); auto.
    clear - H0.
...
Lemma semax_call1:
  forall Espec {cs: compspecs} Delta fs A Pre Post NEPre NEPost
         ts x id argsig retsig cc a bl b fid P Q R
         (Hsub: funspec_sub fs (mk_funspec (argsig, retsig) cc A Pre Post NEPre NEPost)),
    Cop.classify_fun (typeof a) = Cop.fun_case_f (type_of_params argsig) retsig cc ->
    match retsig with
    | Tvoid => False
    ...
    precise_context Delta ->
    (glob_specs Delta) ! fid = Some fs ->
    @semax cs Espec Delta
      ((tc_expr Delta a && tc_exprlist Delta (argtypes argsig) bl) &&
        local (fun rho =>
          gvar_injection (ge_of rho) /\
          eval_expr a rho = Vptr b Ptrofs.zero /\
          global_block rho fid b) &&
       (|> `(Pre ts x: environ -> mpred)
            (make_args' (argsig,retsig) (eval_exprlist (argtypes argsig) bl)) *
        |> PROPx P (LOCALx Q (SEPx R))))
      (Scall (Some id) a bl)
...
Proof.
  intros.
  apply (@semax_call' Espec cs Delta fs A Pre Post NEPre NEPost ts x (Some id) argsig retsig cc a b fid bl P Q R); auto.
Qed.
...
Lemma semax_call0:
  forall Espec {cs: compspecs} Delta fs A Pre Post NEPre NEPost
         ts x argsig retty cc a bl b id P Q R
    ...
    @semax cs Espec Delta
      ...
      (normal_ret_assert
        (ifvoid retty (`(Post ts x: environ -> mpred) (make_args nil nil))
                      (EX v:val,
                        `(Post ts x: environ -> mpred)
                         (make_args (ret_temp::nil) (v::nil))) *
         PROPx P (LOCALx Q (SEPx R)))).
Proof.
  intros.
  rewrite argtypes_eq.
  eapply semax_pre_post'; [ | |
    apply (semax_call_subsume fs A Pre Post NEPre NEPost argsig retty cc Hsub
                Delta ts x (PROPx P (LOCALx Q (SEPx R))) None a bl b id H)
  ].
  3:{
...
Lemma semax_fun_id':
  forall id f TC Espec {cs: compspecs} Delta (PQR: environ->mpred) PostCond c
...
Lemma semax_fun_id1:
  forall {CS: compspecs} {Espec: OracleKind} id f Delta P Q c,
    ...
    semax Delta (P &&
      (EX b,
        local (fun rho =>
          eval_var id (type_of_funspec f) rho = Vptr b Ptrofs.zero /\
          global_block rho id b)))
      c Q ->
    semax Delta P c Q.
Proof.
  ...
  destruct H4 as [? [? ?]].
  assert (Map.get (ve_of rho) id = None).
  { specialize (H5 id).
    rewrite H in H5.
    destruct (Map.get (ve_of rho) id) as [[? ?] |].
    - exfalso. specialize (H5 t). destruct H5.
      specialize (H7 ltac:(eexists; eauto)). congruence.
    - reflexivity. }
  rewrite H7.
  Exists b.
  apply prop_right.
...
Lemma eqb_typelist_refl:
  forall tl, eqb_typelist tl tl = true.
Proof.
...
(* Lemma semax_call_id0:
  forall Espec {cs: compspecs} Delta P Q R b id bl
         fs argsig retty cc A ts x Pre Post NEPre NEPost
         (Hsub: funspec_sub fs (mk_funspec (argsig,retty) cc A Pre Post NEPre NEPost))
         (Hprecise: precise_context Delta)
         (GLBL: (var_types Delta) ! id = None),
    (glob_specs Delta) ! id = Some fs ->
...
Proof.
  ...
  apply (semax_fun_id' id fs _ Espec Delta); auto.

  eapply FloydSeq.canon.semax_pre_simple; [ | apply (@semax_call0 Espec cs Delta fs A Pre Post NEPre NEPost ts x argsig _ cc _ bl b id P Q R Hsub); auto].
  apply andp_right.
  { rewrite <- andp_assoc. apply andp_left1.
    ...
    * intro rho. unfold local, lift1. simpl.
      ...
      apply andp_left2. apply prop_derives.
      intros [? ?].
      split; [| split]; auto.
      destruct H4. unfold eval_var.
...
(* Lemma semax_call_id1:
  forall Espec {cs: compspecs} Delta P Q R ret b id fs
         retty cc bl argsig A ts x Pre Post NEPre NEPost
         (Hsub: funspec_sub fs (mk_funspec (argsig,retty) cc A Pre Post NEPre NEPost))
         (Hprecise: precise_context Delta)
         (GLBL: (var_types Delta) ! id = None),
    (glob_specs Delta) ! id = Some fs ->
    ...
    @semax cs Espec Delta
      ((tc_exprlist Delta (argtypes argsig) bl &&
        local (fun rho =>
          ...
          global_block rho id b)) &&
       |> (`(Pre ts x: environ -> mpred)
            (make_args' (argsig,Tvoid) (eval_exprlist (argtypes argsig) bl)) *
           PROPx P (LOCALx Q (SEPx R))))
      (Scall (Some ret) (Evar id (Tfunction (type_of_params argsig) retty cc)) bl)
      (normal_ret_assert
...
Proof.
  ...
  subst.
  eapply FloydSeq.canon.semax_pre_simple; [ | apply (semax_call1 Espec Delta fs A Pre Post NEPre NEPost ts x ret argsig retty cc _ bl b id P Q R Hsub H1 H0); auto].
  apply andp_right.
  { rewrite <- andp_assoc. apply andp_left1. apply andp_right;[apply andp_right|].
    ...
    * apply andp_left2, andp_left1; trivial.
    * intro rho. unfold local, lift1. simpl.
      apply derives_extract_prop. intros.
      apply andp_left2. apply prop_derives.
      intros [? ?].
...
Lemma isolate_LOCAL_lem1:
  forall Q,
    PROPx nil (LOCALx Q (SEPx (TT::nil))) =
    local (fold_right `(and) `(True) (map locald_denote Q)).
Proof.
  intros.
  extensionality rho.
...
Lemma check_specs_lemma:
  forall Qtemp Qpre_temp Qvar GV GV' rho fl vl
    ...
    fold_right `(and) `(True)
      (map locald_denote (LocalD Qpre_temp (PTree.empty _) GV')) (make_args fl vl rho).
Proof.
  intros.
  apply local_ext_rev.
...
Definition check_retty t :=
  match t with
  | Tvoid => Result_type_in_funspec_different_from_call_statement
...
Lemma PROP_LOCAL_SEP_f:
  forall P Q R f,
    `(PROPx P (LOCALx Q (SEPx R))) f =
    local (fold_right `(and) `(True)
            (map (fun q : environ -> Prop => `q f) (map locald_denote Q))) &&
...
Definition call_setup1
    (cs: compspecs) Qtemp Qvar GV a Delta P Q R
    fs argsig retty cc (A: rmaps.TypeTree) Pre Post NEPre NEPost
    (bl: list expr) (vl : list val)
    (Qactuals : PTree.t _) :=
  local2ptree Q = (Qtemp, Qvar, nil, GV) /\
...
Proof.
  unfold can_assume_funcptr; intros.
  eapply (semax_fun_id1 id); try eassumption.
...
Lemma call_setup2_i:
  forall (cs: compspecs) Qtemp Qvar GV a Delta P Q R R'
    ...
    check_gvars_spec GV GV' ->
    fold_right_sepcon R |-- fold_right_sepcon Rpre * fold_right_sepcon Frame ->
    call_setup2 cs Qtemp Qvar GV a Delta P Q R R' fs argsig retty cc ts A Pre Post NEPre NEPost bl vl Qactuals witness' Frame Ppre Qpre Rpre Qpre_temp GV'.
Proof.
...
Lemma call_setup2_i_nil:
  forall (cs: compspecs) Qtemp Qvar GV a Delta P Q R R'
    ...
    check_gvars_spec GV GV' ->
    fold_right_sepcon R |-- fold_right_sepcon Rpre * fold_right_sepcon Frame ->
    call_setup2_nil cs Qtemp Qvar GV a Delta P Q R R' fs argsig retty cc A Pre Post NEPre NEPost bl vl Qactuals witness' Frame Ppre Qpre Rpre Qpre_temp GV'.
Proof.
  intros. split. auto. repeat split; auto.
Qed.
...
Proof.
  ...
  rewrite <- ! andp_assoc.
  rewrite (andp_comm _ (EX b id, _)).
  rewrite ! andp_assoc.
  rewrite !exp_andp1. Intros b id.
    ...
    |> PROPx P (LOCALx Q (SEPx R)))).
  {
    ...
    apply andp_right. solve_andp. apply andp_left1. apply andp_left2. trivial. }

  forget (tc_exprlist Delta (argtypes argsig) bl) as TCEXPRLIST.
  eapply derives_trans;[ apply andp_derives; [apply derives_refl | apply derives_refl] |].
  subst Qactuals.
...
Lemma tc_exprlist_len:
  forall {cs : compspecs} Delta argsig bl,
    tc_exprlist Delta (argtypes argsig) bl
    |-- !!(length (argtypes argsig) = length bl).
Proof.
  intros.
  go_lowerx.
  unfold tc_exprlist.
...
Lemma semax_pre_setup2
    {cs Espec} Delta fs a bl argsig P Q R' Qactuals Qpre_temp Post2 rv
...
Lemma semax_call_id00_wow:
  forall (cs: compspecs) Qtemp Qvar a GV Delta P Q R R'
         ...
         (vl : list val)
         (SETUP: call_setup2 cs Qtemp Qvar GV a Delta P Q R R' fs argsig retty cc ts A Pre Post NEPre NEPost bl vl Qactuals witness Frame Ppre Qpre Rpre Qpre_temp GV')
         Espec
         (Post2: environ -> mpred)
         (B: Type)
         (Ppost: B -> list Prop)
         ...
         (POST2: Post2 = EX vret:B,
            PROPx (P ++ Ppost vret) (LOCALx Q (SEPx (Rpost vret ++ Frame))))
         (PPRE: fold_right_and True Ppre)
         (Hprecise: precise_context Delta),
    @semax cs Espec Delta
      (PROPx P (LOCALx Q (SEPx R')))
...
Proof.
  intros.
  destruct SETUP as [[PTREE [Hsub [SPEC [ATY [TC0 [TC1 [MSUBST PTREE'']]]]]]]
  ...
  apply semax_extract_prop; intros.
  rewrite andp_comm.
  normalize. apply semax_extract_exists. intro b.
  normalize. apply semax_extract_exists. intro id.
  normalize. apply semax_extract_prop. intros.
  eapply semax_pre_post_allp_fun_id'; [ | |
    apply (@semax_call0 Espec cs Delta fs A Pre Post NEPre NEPost
                ts witness argsig retty cc a bl b id P Q Frame Hsub)]; auto.
  *
    rewrite (andp_comm _ (local (fun _ => gvar_injection _ /\ _))).
    rewrite (andp_assoc (local (fun _ => gvar_injection _ /\ _)) _ _).
    apply andp_right.
    ...
    { apply derives_trans with (
      ...
      { solve_andp. }
      unfold allp_fun_id. intro rho. simpl.
      normalize.
      apply andp_left2. unfold local, lift1, lift0.
      apply prop_derives. intros [? ?].
      split; [| split]; assumption. }

    { eapply derives_trans. 2: eapply semax_call_aux55; eauto.
      Exists b id. normalize. apply andp_right.
      apply prop_right. apply H1.
      solve_andp. }
  *
    subst.
...
Lemma semax_call_id00_wow_nil:
  forall (cs: compspecs) Qtemp Qvar GV a Delta P Q R R'
         ...
         (Qactuals Qpre_temp : PTree.t _) GV'
         (vl : list val)
         (SETUP: call_setup2_nil cs Qtemp Qvar GV a Delta P Q R R' fs argsig retty cc A Pre Post NEPre NEPost bl vl Qactuals witness Frame Ppre Qpre Rpre Qpre_temp GV')
         Espec
         (Post2: environ -> mpred)
         (B: Type)
         (Ppost: B -> list Prop)
    ...
    @semax cs Espec Delta
      ...
      (normal_ret_assert Post2).
Proof.
  intros. rewrite call_setup2_nil_equiv in SETUP.
  eapply semax_call_id00_wow; eassumption.
Qed.
...
Lemma semax_call_id1_wow:
  forall (cs: compspecs) Qtemp Qvar GV a Delta P Q R R'
         ...
         (POST1: Post ts witness = EX vret:B,
            PROPx (Ppost vret)
...
Proof.
  ...
  eapply semax_pre_post_allp_fun_id'; [ | |
      apply (@semax_call1 Espec cs Delta fs A Pre Post NEPre NEPost
             ts witness ret argsig retty cc a bl b id P Q Frame Hsub)];
  [ |
    | assumption
  ...
  *
    ...
    clear.
    go_lowerx. normalize. apply exp_right with x; normalize. solve_andp.
...
Lemma semax_call_id1_wow_nil:
  forall (cs: compspecs) Qtemp Qvar GV a Delta P Q R R'
         ...
         (TYret: typeof_temp Delta ret = Some retty)
         (OKretty: check_retty retty)
         (POST1: Post nil witness = EX vret:B,
            PROPx (Ppost vret)
            (LOCALx (temp ret_temp (F vret) :: nil)
            (SEPx (Rpost vret))))
         (DELETE: remove_localdef_temp ret Q = Qnew)
...
Proof.
  intros. rewrite call_setup2_nil_equiv in SETUP.
  eapply semax_call_id1_wow; eassumption.
Qed.
...
Proof.
  intros. rewrite call_setup2_nil_equiv in SETUP.
  eapply semax_call_id1_x_wow; eassumption.
Qed.
...
Proof.
  intros. rewrite call_setup2_nil_equiv in SETUP.
  eapply semax_call_id1_y_wow; eassumption.
Qed.
...
Proof.
  ...
  eapply semax_pre_post_allp_fun_id';
     [ |
     | apply semax_call0 with (fs:=fs)(cc:=cc)(A:= A) (ts := ts)(x:=witness) (P:=P)(Q:=Q)(NEPre :=NEPre) (NEPost := NEPost)(R := Frame)(b:=b)(id:=id)
     ];
     try eassumption.
  *
    rewrite (andp_comm _ (local (fun _ => gvar_injection _ /\ _))).
  ...
  *
    ...
    rename x0 into vret.
    clear.
    rewrite fold_right_sepcon_app. solve_andp.
Qed.
...
Proof.
  intros.
  rewrite call_setup2_nil_equiv in SETUP.
  eapply semax_call_id01_wow; eassumption.
Qed.

...
let compile_c_ast sourcename csyntax ofile =
  ...
  if !option_C then
    let aclight =
      ...
      | Errors.OK clight2 ->
      begin match BuildAnnotation.annotate_program clight2 with
        | Errors.OK p -> p
...
Processing options:
  ...
  -A             Generate annotated program
  -E             Preprocess only, send result to standard output
  -o <file>      Generate output in <file>
  -V <name>      Import given name at the beginning of generated .v file
|} ^
prepro_help ^
...
(* Processing options *)
 [ Exact "-A", Set option_C;
  ...
  Prefix "-o", Self (fun s -> let s = String.sub s 2 ((String.length s) - 2) in
                              option_o := Some s);
  Exact "-V", String(fun s -> ExportAClight.option_V := s :: !ExportAClight.option_V);]

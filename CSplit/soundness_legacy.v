...
Definition conj_rule := semax_conj_rule.
...
Lemma split_not_empty: forall 
  s_stm (c_stm: C_statement s_stm)
  (c_res: C_result (S_split s_stm)), 
  C_split s_stm c_stm = c_res ->
...
Proof.
  ...
  - simpl in H.
    ...
    { eapply semax_noreturn_inv in H1;[apply H1|..];eauto.
...
Proof.
  ...
  - induction pres.
    { simpl in H1. rewrite app_nil_r in H1.
      ...
      eapply semax_conseq;[..|apply semax_skip];
    ...
    }
    {
      ...
      assert (E2: (add_post_to_semax Delta) (EX Q:assert, Q &&
                        !! (add_pre_to_semax Delta Q)(Basic_partial a, p3))
                                    (Basic_partial a1,p1)).
      { inv E1. rename H4 into E1'. apply path_to_statement_app_aux in E1'.
        apply semax_seq_inv' in E1'. simpl in E1'.
        eapply semax_post_simple;[..|apply E1'];intros;
          try apply derives_refl.
        simpl. intros. Intros Q. Exists Q.
        apply andp_right;[apply derives_refl|]. apply prop_right.
        auto.
...
Proof.
  ...
  induction a1.
  - inv H. rename H1 into H'. 
    apply path_to_statement_app_aux in H'. apply semax_seq_inv in H'. destruct H' as [Q [H1 H2]].
  ...
  - simpl in H. inv H. intros x. 
    specialize (H5 x). apply inj_pair2 in H2. subst. simpl.
    apply H0 in H5. auto.
Qed.
...
Proof.
  ...
  - exfalso. apply H0. auto.
  - induction pres.
    { eapply semax_post_simple;[..|apply semax_skip];
...
Proof.
  ...
  apply semax_seq_inv' in H. unfold overridePost in H.
  eapply atom_to_semax_derives with (Q1:=
   (EX Q : environ -> mpred,
                       !! semax Delta Q 
...
Proof.
  ...
  + rename H into H1. simpl in H1.
    ...
    rewrite !andp_assoc.
    apply semax_extract_prop. intros E3.
    rewrite andp_comm.
...
Proof.
  ...
  + rename H into H1. simpl in H1.
    ...
    apply semax_extract_prop. intros E3.
    rewrite andp_comm.
    rewrite !exp_andp1. apply semax_extract_exists. intros P'.
    rewrite andp_assoc.
    apply semax_extract_prop. intros [E1 E2].
    rewrite andp_comm.
    eapply semax_conseq;[..|apply H2].
...
Proof.
  ...
  - rename H into H1. apply semax_seq_inv in H1. destruct H1 as [Q' [? ?]].
...
Proof.
  intros. inv H. apply inj_pair2 in H1. subst.
  revert x. auto.
Qed.
...
Proof.
  ...
  - simpl in *. intros. apply H0.
    inv H. apply inj_pair2 in H2. subst. auto.
  (* auto. *)
Qed.
...
} (*end of part 1*)
{
  ...
  + (* Basic Case *)
    ...
    - inv H5. rename H11 into H5. 
      ...
      eapply semax_post_simple;[..|apply H5].       
      * apply derives_refl.
    ...
    - inv H6. rename H11 into H6.
      ...
      { unfold RA_normal. 
        ...
        apply semax_break. }
    - inv H5. rename H11 into H5.
      eapply semax_conseq;[..|apply semax_skip];unfold normal_ret_assert, RA_normal,
      RA_return,RA_break,RA_continue.
      { apply semax_skip_inv in H5.
    ...
    - inv H8. rename H11 into H9.
      ...
      eapply semax_conseq;[..|apply H9];unfold normal_ret_assert, RA_normal,
      RA_return,RA_break,RA_continue.
      { apply derives_aux_refl. }
      { apply andp_left2.
        apply derives_extract_PROP. intros C;destruct C. }
      ...
      { apply andp_left2.
        apply derives_extract_PROP. intros C;destruct C. }
      { intros. apply derives_aux_refl. }
    - inv H5. rename H11 into H5. apply semax_seq_skip in H5.
      eapply semax_conseq;[..|apply H5];unfold normal_ret_assert, RA_normal,
...
+ (** Loop *)
  ...
  eapply semax_conseq with (R':=Q);[apply H3'|..];intros;try apply derives_aux_refl.
...
+ (* one loop invariant.*)
  ...
  { simpl. intros.
   ...
   {  (* inv2 c2 Q *)
      ...
      - in_split_result S4.  
      - apply Forall_forall. intros.
        apply semax_extract_exists. intros inv.
        rewrite andp_comm. apply semax_extract_prop.
        intros. 
        assert(Easy: atom_to_semax Delta inv (EX Q' : assert,
               Q' &&

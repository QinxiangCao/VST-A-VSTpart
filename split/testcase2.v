...
Module Info.
  ...
  Definition source_file := "cprogs/sgn.c"%string.
...
Definition _sgn : ident := 55%positive.
...
((___builtin_bswap64,
 ...
 (___builtin_debug,
   Gfun(External (EF_external "__builtin_debug"
     ...
     {|cc_vararg:=true; cc_unproto:=false; cc_structret:=false|})) ::
 (_sgn, Gfun(Internal f_sgn)) :: nil).

Definition public_idents : list ident :=
(_sgn :: ___builtin_debug :: ___builtin_nop :: ___builtin_write32_reversed ::
...
(* (Sgiven Z 0 (fun x => (
    ( PROP (Int.min_signed <= x <= Int.max_signed) LOCAL (temp _x  (Vint (Int.repr x))) SEP ())%assert))
    (fun x => *)
    ((
    (Ssequence

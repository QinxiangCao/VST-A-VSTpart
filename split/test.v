...
Module Info.
  ...
  Definition source_file := "cprogs/append.c"%string.
...
Goal forall Delta, strong.precise_funspec_logic Delta (snd append_spec).
Proof.
  intros.
  hnf. intros.
  Intros ts1 x1. Intros ts2 x2.
  destruct x1 as [[[[sh x] y] s1] s2].
  destruct x2 as [[[[sh' x'] y'] s1'] s2'].
  Exists ts1 ((Share.glb sh sh'), x, y, s1, s2).

  Print writable_share.

  Print nonempty_share.

  Print sepalg.nonidentity.

  Print sepalg.identity.
